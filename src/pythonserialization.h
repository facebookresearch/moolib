...
struct PickleModule {
  ...
  PyMethodDef fileMethods[5] = {{"write", (PyCFunction)&File::write, METH_O, ""},
  ...
  PickleModule() {
    ...
    pickler = pickle.attr("Pickler")(py::handle(o), pickle.attr("HIGHEST_PROTOCOL"));
    dump = pickler.attr("dump");

    unpickler = pickle.attr("Unpickler")(py::handle(o));
    load = unpickler.attr("load");
  }
