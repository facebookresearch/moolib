...
struct AccumulatorImpl {
  ...
  py::object deepCopy(const py::handle& v) {
    if (v.ptr() == Py_True) {
      ...
      fatal("Don't know how to copy python type %s", std::string(py::str(v.get_type())));
    }
  }
  ...
  std::vector<rpc::Tensor> allocateGradients() {
    ...
    for (rpc::Tensor& v : h->modelParameters) {
      ...
      if (v.requires_grad()) {
        if (!grad.defined()) {
          // fmt::printf("Setting grad on a tensor!\n");
  ...
  void reduceImpl(int batchSize) {
    ...
    if (batchSize) {
      ...
      // if (!target->data.gradients.empty()) {
      //  fatal("internal error gradients not empty");
      //}
    }
    if (synchronize && stream) {
      stream->synchronize();
    ...
    try {
      ...
      if (target->syncId == h->syncId && target->syncId == group->syncId) {
        ...
        //            float sum = 0.0f;
        //            for (auto& v : vec) {
        //              sum += v.sum().template item<float>();
        //            }
        //            return sum;
        //          };
        //          log.debug("Reduce data: batch %d/%d (%d grads, %d skipped) grads sum %g\n", target->data.batchSize,
        //          minBatchSize, target->data.numGradients, target->data.numSkipped, sum(target->data.gradients));
...
Accumulator::Accumulator(std::string name, pybind11::object parameters, pybind11::object buffers) {
  impl = std::make_unique<AccumulatorImpl>(name, parameters, buffers);
}

Accumulator::Accumulator(const Group& group, std::string name, pybind11::object parameters, pybind11::object buffers) {
