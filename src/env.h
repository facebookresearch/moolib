...
std::mutex eprofileMutex;
std::map<std::string_view, float> eprofileTimes;

struct eProfile {
  std::string_view name;
  Timer t;
  bool running = true;
  eProfile(std::string_view name) : name(name) {}
  ~eProfile() {
    stop();
  }
  void stop() {
    if (running) {
      running = false;
      std::lock_guard l(eprofileMutex);
      eprofileTimes[name] += t.elapsed();
    }
  }
};
...
struct Env {
  ...
  void step(Shared* shared, size_t bufferIndex, size_t batchIndex) {
    eProfile ptotal("step total");
    eProfile pwait("step wait");
    ++steps;
...
struct EnvRunner {
  ...
  void run(std::string serverAddress) {
    ...
    float lastPrint = mainTimer.elapsed();
    ...
    while (true) {
      ...
      if (!in.queue.empty()) {
        ...
        if (nSteps != stepsDone) {
          ...
          pwait.stop();
          // log.info("myIndex %d doing %d steps with buffer index %d\n", myIndex, nSteps - stepsDone, bufferIndex);
          batch.at(bufferIndex).step(nSteps - stepsDone, &shared, bufferIndex, offset);
          pwait = eProfile("run wait");
          output->nStepsOut.store(nSteps);
          shared.clientOut[myIndex].semaphore.post();

          float now = mainTimer.elapsed();
          if (now - lastPrint >= 2) {
            lastPrint = now;
            ...
            std::string s;
            std::lock_guard l(eprofileMutex);
            for (auto& [key, value] : eprofileTimes) {
              if (!s.empty()) {
                s += " ";
              }
              s += key;
...
struct EnvStepper {
  ...
  std::array<std::map<std::string, rpc::Tensor>, maxBuffers> outputPinned;
